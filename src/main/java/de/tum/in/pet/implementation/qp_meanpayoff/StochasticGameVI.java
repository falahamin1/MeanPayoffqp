...
public class StochasticGameVI {
    ...
    {
        return rounded(reward + (upperboundsP2_i1.get(nextstate)*tau*tau + tau * (1-tau) * upperboundsP1_i1.get(currstate))/tau);

    }

    private double getLValueP1( int currstate, int nextstate, double reward )
    {
        return rounded(reward + (lowerboundsP2_i1.get(nextstate)*tau*tau + tau * (1-tau) * lowerboundsP1_i1.get(currstate))/tau);
